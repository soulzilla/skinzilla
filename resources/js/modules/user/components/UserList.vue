<template>
    <section id="user">
        ...
        <el-table
            ...
            <el-table-column prop="name" label="Name" min-width="200" sortable>
                <template slot-scope="scope">
                    <router-link class="el-link el-link--default ellipsis-form" :to="{name: 'Show User', params: {id: scope.row.id}}">
        ...
        <el-dialog
            ...
            class="users-dialog">
            <UserForm :initialForm="formData" @saved="saved" @cancel="formVisible = false" />
        </el-dialog>
    </section>
</template>
...
<script>
    ...
    import {USER_FETCH, USER_OBTAIN} from "../store/types";
    ...
    export default {
        ...
        computed: {
            ...mapGetters(['users', 'usersMeta', 'usersLoading']),
        },
        created() {
            this.fetchData()
        },
        ...
        methods: {
            ...
            handleDelete(row) {
                this.$confirm('This will permanently delete the user. Continue?', 'Warning', {
                    ...
                    userApi.delete(row.id).then((response) => {
                        this.$message({

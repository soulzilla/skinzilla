<template>
    <div class="my-3">
                ...
                <template v-else>
                    <b-card class="shadow" footer-tag="footer">
                        ...
                        <template v-else>
                            ...
                            <b-card-body>
                                <Rating v-if="$auth.check()" :item="item" :entity_table="'gamblings'"/>
                                ...
                                <div class="row mt-3">
                                    <div class="col-6 px-1 border-right text-center">
                                        ...
                                        <a :href="item.ref_link"
                                           ...
                                           :id="'gambling-bonus-' + item.id">
                                            Регистрация
                                        </a>
                                        <b-popover :target="'gambling-bonus-' + item.id" triggers="hover">
                                            Зарегистрируйтесь по нашей ссылке и получите бонус!
                                        </b-popover>
                                    </div>
                                    <div class="col-6 px-1 text-center">
                                        <p>Для всех пользователей</p>
                                        <button class="btn btn-primary" @click="copyToClipboard(item.promo_code)" :id="'gambling-promo-' + item.id">{{ item.promo_code }}</button>
                                        <b-popover :target="'gambling-promo-' + item.id" triggers="hover">
                        ...
                        <template #footer class="bg-white">
                            <ItemFooter :item="item" :entity_table="'gamblings'"/>
                        </template>
                    </b-card>
                </template>
            </div>
        ...
        <div class="my-3" v-if="!load">
            <Comments :entity_id="item.id" :entity_table="'gamblings'"/>
        </div>
    </div>
</template>
...
export default {
    ...
    mounted() {
        var that = this
        gamblingApi.one(this.$route.params.id).then(function (response) {

<template>
    <section id="roulette">
        ...
        <el-table
            ...
            <el-table-column prop="name" label="Name" min-width="200" sortable>
                <template slot-scope="scope">
                    <router-link class="el-link el-link--default ellipsis-form" :to="{name: 'Show Roulette', params: {id: scope.row.id}}">
        ...
        <el-dialog
            ...
            class="roulettes-dialog">
            <RouletteForm :initialForm="formData" @saved="saved" @cancel="formVisible = false" />
        </el-dialog>
    </section>
</template>
...
<script>
    ...
    import {ROULETTE_FETCH, ROULETTE_OBTAIN} from "../store/types";
    import rouletteApi from '../api'
    import RouletteForm from "./RouletteForm";

    export default {
        name: "RouletteList",
        components: {RouletteForm},
        data: () => ({
            sortBy: 'id,asc',
            ...
            page: 1,
            formVisible: false,
            formTitle: 'New Roulette',
            formData: null
        }),
        computed: {
            ...mapGetters(['roulettes', 'roulettesMeta', 'roulettesLoading']),
        },
        created() {
            this.fetchData()
        },
        ...
        methods: {
            ...mapActions([ROULETTE_FETCH]),
            ...mapMutations([ROULETTE_OBTAIN]),
            ...
            fetchData() {
                ...
                this[ROULETTE_FETCH](params)
            },
            handleAdd() {
                this.formTitle = 'New Roulette';
                this.formData = {};
                this.formVisible = true;
            },
            handleEdit(row) {
                this.formTitle = 'Edit Roulette';
                this.formData = Object.assign({}, row);
                this.formVisible = true;
            },
            handleDelete(row) {
                this.$confirm('This will permanently delete the roulette. Continue?', 'Warning', {
                    type: 'warning'
                }).then(() => {
                    rouletteApi.delete(row.id).then((response) => {
                        this.$message({
